# Purpose: Estimation of a Bayesian Gaussian-Poisson joint model to derive R²_lambda 
...
bayes_model_func <- function(x = unique(data_delta$study),
                             ...
                             gaus = "t2_volume",
                             time = "day_SE",
                             path = "results_gaus_poisson/delta")
{
  library(lme4)
  library(pbapply)
  library(parallel)
  ...
  for(comp in 1:nrow(comparisons))
  {
    ...
    FP_poisson <-   fp_func(Covariate = "year",
                         Outcome = poisson,
                         id = id,
                         Max.M = 2,
                         S = c(-2,-1,-0.5,0,0.5,1,2,3),
    ...
    formula_poisson <- paste0(formula_poisson," + (1|c|obs)") # observational,correlated random effect (see brms paper for details)
    ...
    formula_gaus <- paste0(formula_gaus," + (1|c|obs)")
    formula_gaus <- bf(formula = formula_gaus,
                     family = gaussian(),
                     sigma = 0.1) # Sigma is set to 0.1
