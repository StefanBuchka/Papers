...
data_SE <- mri %>%
  ...
  filter(visit_com != 60) %>%
  filter(visit_com != 48) %>% # in CAMMS223 relapses are be considered untiel 3 years after study start. 
  # So, only t2 volume until three years after study start is considered.
...
bayes_model_func <- function(x = unique(data$study),
                             ...
                             SE = "t2_volume",
                             TE = "relapse",
                             path = "results_gaus_count_ITassociative")
{
  library(lme4)
  library(pbapply)
  library(parallel)
