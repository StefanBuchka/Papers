...
# Purpose of this script: Simulation study of Poisson SEP and Gaussian CEP
...
if(!file.exists("poisson_gaus"))
{dir.create("poisson_gaus")}

if(!file.exists("poisson_gaus/MA"))
{dir.create("poisson_gaus/MA")}
                             ...
                             path = "poisson_gaus/MA")
{
  ...
  formula_poisson <- bf(formula = paste0(poisson,"~ offset(log(year_SE)) + year_SE + (1|c|obs)"),
                         family = poisson())
  
  
  formula_gaus <- bf(formula = paste0(gaus,"~ year_TE + (1|c|obs)"),
                      family = gaussian(),
                      sigma = 0.1)
  ...
  error = function(e)
  {
    ...
    save(errors,file = paste0(path,"/errors/",x,".Rdata"))
  },
  warning = function(w)
  {
    # Catches warnings
    ...
    save(warn,file = paste0(path,"/warnings/",x,".Rdata"))
...
if(!file.exists("poisson_gaus/IT"))
{dir.create("poisson_gaus/IT")}
...
if(!file.exists("poisson_gaus/IT_iter"))
{dir.create("poisson_gaus/IT_iter")}
