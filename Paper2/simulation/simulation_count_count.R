# Purpose of this script: Simulation study of Poisson SEP and Poisson CEP
...
if(!file.exists("poisson_poisson"))
{dir.create("poisson_poisson")}

if(!file.exists("poisson_poisson/MA"))
{dir.create("poisson_poisson/MA")}
                             ...
                             path = "poisson_poisson/MA")
{
  ...
  # dplyr::select study within function
  ...
  formula_TE <- bf(formula = paste0(TE,"~ offset(log(year_TE)) + year_TE + (1|c|obs)"),
                    family = poisson())
  
  formula_SE <- bf(formula = paste0(SE,"~ offset(log(year_SE)) + year_SE + (1|c|obs)"),
                    family = poisson())
  
  
  # Estimation of the model
  tryCatch({
    model <<- brm(formula_SE + formula_TE + set_rescor(F),
                  iter = 15000,
                  chains = 4,
                  cores = 4,
...
if(!file.exists("poisson_poisson/IT"))
{dir.create("poisson_poisson/IT")}
                             ...
                             path = "poisson_poisson/IT")
{
  ...
  tryCatch({
    ...
    model_1_zi <- glmmTMB(as.formula(paste0(TE, "~ offset(log(year_SE)) +",SE,"+ year_TE")),
                          family = "poisson",
                          REML = T,
                          control = glmmTMBControl(optimizer = optim,
                                                   optArgs = list(method = "BFGS")),
  ...
  tryCatch({
    data_loop$t2_cat <- as.factor(as.numeric(cut_number(data_loop$Sp,2)))
  ...
  tryCatch({
    ...
    model_1_or <- clmm2(Tp ~ t2_cat + year_TE ,
                        Hess = T,
                        data = data_loop)
  },
  error = function(e)
...
if(!file.exists("poisson_poisson/IT_iter"))
{dir.create("poisson_poisson/IT_iter")}
