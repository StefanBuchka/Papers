# Purpose: Estimation of a Bayesian Poisson-Gaussian joint model to derive R²_lambda 
...
# Data about Relapses
data_TE <- arr %>%
  select(study,id,arm,relapse,day) %>%
...
# data about MRI
data_SE <- mri %>%
  select(study,id,arm,t2_new_or_enlarged,day,visit,visit_com) %>%
...
# The baseline t2 count were removed, since the base line count are the numbers of t2 lesions at base line. This is the new or enlarged lesion count. 
...
bayes_model_func <- function(x = unique(data_delta$study),
                             ...
                             TE = "rel",
                             time = "day_SE",
                             path = "results_poisson_poisson/delta")
{
  library(lme4)
  library(pbapply)
  library(parallel)
  ...
  for(comp in 1:nrow(comparisons))
  {
    ...
    # Create poisson formulas for model
    ...
    formula_TE <- paste0(formula_TE," + (1|c|obs)")
    ...
    formula_SE <- paste0(formula_SE," + (1|c|obs)")
    ...
    res_SE <- res[,,"t2neworenlarged_Intercept"]
