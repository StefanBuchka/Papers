...
# Purpose of this script: Simulation study of Poisson SEP and Poisson CEP (transformed Poisson endpoints)
...
if(!file.exists("poisson_poisson_t"))
{dir.create("poisson_poisson_t")}

if(!file.exists("poisson_poisson_t/MA"))
{dir.create("poisson_poisson_t/MA")}
...
if(!file.exists("poisson_poisson_t/IT"))
{dir.create("poisson_poisson_t/IT")}
...
if(!file.exists("poisson_poisson_t/IT_iter"))
{dir.create("poisson_poisson_t/IT_iter")}
...
bayes_model_func <- function(x = unique(data_i[[1]]$study),
                             ...
                             SE = "St",
                             time = "year_TE",
                             path = "poisson_poisson_t/IT_iter")
{
  library(lme4)
  library(pbapply)
  library(parallel)
  ...
  for(i_data in 1:length(data))
  {
    ...
    if(length(unique(data_loop$T)) == 1)
    {
      data_loop <- data_loop %>%
        mutate(#`T` = abs(`T`),
          `T` = ifelse(`Tt` >= 1.3,2,`Tt`),
          `T` = floor(`T`*2)/2,
          year = year_TE)
    }
