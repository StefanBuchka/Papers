...
data_delta <- merge(data_SE,
  ...
  mutate(day_SE = ifelse(day_SE == min(day_SE,na.rm = T) &
                           !is.na(relapse) &
                           visit_com == 0,0,day_SE), #replace the first day of visit by 0, to included relapses happens before the first MRI visit in the interval.
...
bayes_model_func <- function(x = unique(data_delta$study),
                             ...
                             gaus = "t2_volume",
                             time = "day_SE",
                             path = "results_gaus_poisson_t/delta")
{
  library(lme4)
  library(pbapply)
  library(parallel)
  ...
  for(comp in 1:nrow(comparisons))
  {
    ...
    model_poisson <- MA_model_sel_func(data_loop = data_loop,
                                       Outcome = poisson,
                                       term = terms,
                                       family = "gaus") # Its a Gaussian model
    ...
    res_SE <- res[,,gsub("_","",gaus)]
    res_TE <- res[,,gsub("_","",poisson)]
